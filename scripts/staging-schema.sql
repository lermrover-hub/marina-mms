...
CREATE TABLE IF NOT EXISTS public.mms_quotations (
  ...
  status text DEFAULT 'DRAFT'::text NOT NULL,
  internal_approval_status text DEFAULT 'NOT_SUBMITTED'::text NOT NULL,
  internal_approved_by text,
  internal_approval_role text,
...
CREATE TABLE IF NOT EXISTS public.mms_ramp_bookings (
  ...
  service_option text,
  billing_cycle text,
  recurring_billing boolean DEFAULT false NOT NULL,
  next_billing_date date,
  pricing_adjustment_type text DEFAULT 'NONE'::text NOT NULL,
  pricing_adjustment_pct numeric(6,2) DEFAULT 0 NOT NULL,
